...
describe(`CalcAction`, () => {
  ...
  describe(`constructor`, () => {
    it(`should instantiate without error and initialize default values`, () => {
      const calcAction = new CalcAction({ words: [] }, [{}]);
  ...
  describe(`execute`, () => {
    it(`should successfully replace a variable`, async () => {
      const calcAction = new CalcAction({ words: ['BTC_price_usd'] }, new Coins({ symbol: 'BTC', price_usd: 4000 }));
    ...
    it(`should successfully handle parenthesis around a variable`, async () => {
      const calcAction = new CalcAction({ words: ['(BTC_price_usd', '*', '5.0)'] }, new Coins({ symbol: 'BTC', price_usd: 4000 }));
      const message = await calcAction.execute();
